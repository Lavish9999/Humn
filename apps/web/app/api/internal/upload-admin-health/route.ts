...
export async function GET() {
  try {
    ...
    const ok = !original.error && !display.error && !signedUpload.error && Boolean(signedUpload.data?.token);
    return NextResponse.json({
      ...
      signedUpload: { ready: Boolean(signedUpload.data?.token) && !signedUpload.error, errorClass: signedUpload.error?.name ?? null },
    }, { status: ok ? 200 : 503 });
  } catch (error) {
    if (error instanceof SupabaseAdminConfigurationError) {
      return NextResponse.json({ ok: false, errorClass: error.name, errorCode: error.code, keySource: error.keySource }, { status: 503 });
    }
    return NextResponse.json({ ok: false, errorClass: error instanceof Error ? error.name : 'UnknownError' }, { status: 500 });
  }
}
